...
test_Realdata <- test_Realdata %>% mutate_at(c("ADOS",
                                     "MullenRaw",
                                     "ExpressiveLangRaw",
                                     "Socialization",
                                     "MOT_MLU",
...
test_Realdata <- test_Realdata %>% 
  mutate( Predm1 = predict(mlu_realdata_m1, newdata = test_Realdata, allow_new_levels = T),
          Predmados = predict(mlu_realdata_m_ados, newdata = test_Realdata, allow_new_levels = T),
          Predmelr = predict(mlu_realdata_m_ELR, newdata = test_Realdata, allow_new_levels = T )) %>% 
  drop_na(CHI_MLU)
...
```
The 95% credibility intervals are too wide to make a decision, so the point estimates were considered.
With the baseline model, 3 children with autism would be advised to receive speech therapy, 1 out of those 3 would not need it, and none of the typically developing children would be advised to receive speech therapy, even though 2 might need it.
The model with Ados performs the same.
With the added measure of linguistic IQ, only the 2 children with autism, who might need it, would be advised to receive speech therapy, and 2 of the typically developing children would be advised to receive speech therapy, who also might need it.
Based on all methods of model comparison, the model with mean length of utterance, and language expressiveness seems the best for predictions.

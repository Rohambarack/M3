...
test_Realdata_s <- bake(rec_stan, new_data = teszt_test_Realdata)
...
The baseline model would miscategorise 3 children.
The ados model would miscategorise 3 children.
The ELr model would miscategorize 2 children.
I also like, that the elr model's 95% credibility intervals are not touching the "normal" range, meaning the model is telling, these children definitely need speech therapy. The other two models are not so definitive.
...
#possible first visit mlu values range from
max(Realdata$CHI_MLU)

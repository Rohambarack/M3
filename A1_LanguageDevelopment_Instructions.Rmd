...
```{r}
remove.packages(c("Rcpp","dplyer"))
install.packages(c("Rcpp","dplyer"))
library(tidyverse)
library(brms)
library(gridExtra)
library(cmdstanr)
...
parafullp <-
  parafull %>% 
  mutate(TDwidth = Q97.5.y - Q2.5.y,
         ASDwidth = Q97.5.x - Q2.5.x,
         diff = Q2.5.y-Q97.5.x)

parafullp %>% 
  mutate(TDcheck = ifelse(TDwidth < .2, 1, 0),
         ASDcheck = ifelse(ASDwidth < .2, 1, 0)) %>% 
  summarise(`TDprecision` = mean(TDcheck), ASDprecision =mean(ASDcheck), avgtd =mean(TDwidth),avgasd =mean(ASDwidth))
...
fos <- steszt %>% 
 ...
 steszt %>% 
   filter(seed == 1) %>% 
   ggplot()+
  geom_density(aes(`b_grouptd.visit`), fill="steelblue", color="black",alpha=0.6) +
  geom_density(aes(`b_groupasd.visit`), fill="#FC4E07", color="black",alpha=0.6) 
...
steszt2 %>% 
  ...
  labs(x = "seed (i.e., simulation index)",
       ...
       color = "Legend",
       #additional labs
       subtitle = str_glue("30 participants from each group, power = {mean(steszt2$power)}"))+
  scale_color_manual(values = lab_colors)
...
steszt4_full <-merge(steszt4_1,steszt4_2, by = "seed")

```

```{r}

steszt3<-steszt4_full %>% 
...
steszt3 %>% 
  ...
  labs(x = "seed (i.e., simulation index)",
       y = " ",
       color = "Legend",
       subtitle = str_glue("30 participants from each group, power = {mean(steszt3$power)}"))+
  scale_color_manual(values = lab_colors)
...
```{r}
setwd(".")
write_csv(parafull,"parafull.csv")
write_csv(steszt,"draws_data_teszt.csv")
```
...
length(unique(Realdata$SUBJ))
# n = 66
# with 60 it was 0.7 so should be enough

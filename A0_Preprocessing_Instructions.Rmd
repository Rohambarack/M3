...
```{r}
setwd("C:/github/M3/M3_a0")

```

```{r}
data1 <- read_csv("demo_train.csv")
data2 <- read_csv("LU_train.csv")
data3 <- read_csv("token_train.csv")
...
```{r}
data2$VISIT <-  data2$VISIT %>% 
  substr(1,nchar(data2$VISIT[1])-1) %>% 
  substr(6,nchar(data2$VISIT[1])) %>% 
  as.integer()
 

data3$VISIT <-  data3$VISIT %>% 
  substr(1,nchar(data3$VISIT[1])-1) %>% 
  substr(6,nchar(data3$VISIT[1])) %>% 
  as.integer()
...
data1$ID <- paste(data1$SUBJ,data1$VISIT, sep= "_")
data2$ID <- paste(data2$SUBJ,data2$VISIT, sep= "_")
data3$ID <- paste(data3$SUBJ,data3$VISIT, sep= "_")
```
...
df_ultimate2 <- merge(merge(data1, data2, by = "ID", all = TRUE),data3, by = "ID", all = TRUE) %>% select(ID,
SUBJ, 
VISIT, 
Diagnosis, 
Ethnicity, 
...
```{r}
dfulttna <- df_ultimate2 

for (x in dfulttna$SUBJ) {

if(is.na(x)){
  
  dfulttna$SUBJ <- dfulttna$ID %>% 
   substr(1,nchar(dfulttna$ID)-2)
  
  
}
}

for (x in dfulttna$VISIT) {

if(is.na(x)){
  
  dfulttna$VISIT <- dfulttna$ID %>% 
   substr(nchar(dfulttna$ID),nchar(dfulttna$ID)) %>% 
  as.integer()
...
```{r}
x <- 1
while (x<length(dftest$VISIT)+1){

if (dftest$VISIT[x] == 1) { 
  
  y <- dftest$ADOS[x]
  y_1 <- dftest$MullenRaw[x]
  y_2 <- dftest$ExpressiveLangRaw[x]
  y_3 <- dftest$Socialization[x]
  
  
 
} 
  dftest$MullenRaw[x] <- y_1
  dftest$ExpressiveLangRaw[x] <- y_2
  dftest$Socialization[x] <-y_3
   dftest$ADOS[x] <- y
   x <- x+1
...
```{r}
x_2 <- 1
while (x_2<length(dftest$VISIT)+1){

if (dftest$VISIT[x_2] == 1) { 
...
```{r}
dftest$ID <- paste(dftest$SUBJ,dftest$VISIT, sep= "_")
...
df_stor <- cleanthat(data1,data2,data3)

write.csv(df_stor,"comb_data.csv",row.names = FALSE)

```
```{r}
testtest <- read_csv("comb_data.csv")
```
